...
Some papers distinguish skidding from slipping, which is found by comparing the direction of the center of the hoop's velocity, *v\_x*, to the direction of the force of kinetic friction. My code prints both directions, but I prefer to just call all of it *sliding*. Then, In all of the cases I have checked, if the hoop starts with the edge mass straight down and rolling without sliding, the behavior splits into only two simple cases...
...
Another equivalent derivation method is to apply Newton's 2nd law and Newton's 2nd law for rotation to the *center of mass* (CM). For a rigid body, you can always use the CM as its pivot even if it is accelerating. That is, you can take torques about that body's CM and use *τ_CM* = *I_CM* *α* even when the CM is accelerating. Whenever using Newton's 2nd law, the acceleration is the acceleration of the object's CM. 

Whatever you do, I recommend that you check your rolling-without-sliding equation of motion with what you get from Lagrangian mechanics. To do this, define *L*, the Lagrangian, as kinetic energy minus potential energy, and write it so that *θ* and its time derivative (*ω*) are the only time-dependent variables that appear. Then, d(∂*L*/∂*ω*)/dt = ∂*L*/∂*θ* gives the equation of motion. Another way to derive the rolling-without-sliding equation of motion is to write down the energy equation then take the time derivative of it.

Keep in mind that, whenever calculating kinetic energy, it only cleanly separates into rotational + translational if both terms are computed relative to the CM. So, for convenience, for everything but the hop, I find the kinetic energy of each object separately then add them together.
...
Any hop starts when *F*\_normal becomes negative. At takeoff, the combined CM keeps its velocity from just before takeoff, and the object then undergoes projectile motion until the center of the hoop, not the CM, reaches its original height again. The rotational speed, *ω*, does not change at takeoff. You can prove this using conservation of energy or conservation of angular momentum; for that proof, I recommend using a frame of reference moving at *v\_x* relative to the ground with origin at the center of mass.

As for landing a hop, there are comments in the code with all of the mathematical details. The basic idea is, since I assume that collision time is negligible, to treat the vertical impulse like time. That is, I integrate through the collision using the vertical impulse as the independent variable instead of ordinary time.

The uniquely tricky part of simulating a hopping hoop was realizing that sliding always occurs before a hop, even when the coefficient of static friction, *μ\_s*, is infinite. When first writing the code, I assumed an infinite *μ\_s* so that I would not have to worry about sliding yet. I reasoned that *μ*\_*s* *F*\_normal - | *F*\_friction | would never become negative if *μ\_s* is infinite. However, I would get a negative acceleration of the center of the hoop at the very beginning of any hop, which is a contradiction with the hoop hopping. The correct way to take a limit is to have a physical (in this case, non-infinite) value, solve the problem, *then* take the limit. When doing this, we can see that, assuming nonzero *F*\_friction, *μ*\_*s* *F*\_normal - | *F*\_friction | will always become negative before *F*\_normal becomes negative. Taking the limit as *μ\_s* becomes infinite, sliding and hopping both occur at the same time, but we now see that *sliding* is what results, not just hopping.
